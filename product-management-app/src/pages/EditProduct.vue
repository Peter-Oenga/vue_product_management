  ...
  <div class="min-h-screen bg-gray-50 px-10 py-10">
    ...
    <div class="bg-white mt-10 p-10 rounded-2xl shadow-sm max-w-5xl">
      ...
      <div
        ...
        class="border-2 border-dashed border-gray-300 rounded-xl bg-gray-50 py-12 flex flex-col items-center justify-center cursor-pointer hover:border-blue-900 transition mb-6"
      >
        <!-- Upload Icon -->
        <svg
      ...
      <!-- Preview -->
      <div v-if="thumbnail" class="mb-6">
        <p class="font-semibold mb-2 text-sm text-gray-700">Current Image:</p>
        <img :src="thumbnail" class="w-48 rounded-lg border" />
      </div>
...
onMounted(async () => {
  ...
  thumbnail.value = product.value.thumbnail || product.value.images?.[0];
});
...
const updateProduct = async () => {
  ...
  try {
    ...
    errorMessage.value = "Failed to update product. Try again.";
  }
